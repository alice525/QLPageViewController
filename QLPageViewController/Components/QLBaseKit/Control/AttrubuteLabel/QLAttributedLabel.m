...
#define KQLLineBreakWordWrapTextWidthScalingFactor (M_PI / M_E)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"

NSString * const KQLStrikeOutAttributeName = @"QLStrikeOutAttribute";
NSString * const kQLBackgroundFillColorAttributeName = @"QLBackgroundFillColor";
NSString * const kQLBackgroundStrokeColorAttributeName = @"QLBackgroundStrokeColor";
NSString * const kQLBackgroundLineWidthAttributeName = @"QLBackgroundLineWidth";
NSString * const kQLBackgroundCornerRadiusAttributeName = @"QLBackgroundCornerRadius";
...
static inline NSDictionary * NSAttributedStringAttributesFromLabel(QLAttributedLabel *label) {
...
@implementation QLAttributedLabel {
@private
               ...
               context:(CGContextRef)c
{
    ...
    if (self.verticalAlignment != QLAttributedLabelVerticalAlignmentTop) {
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            ...
            CGColorRef strokeColor = (__bridge CGColorRef)[attributes objectForKey:kQLBackgroundStrokeColorAttributeName];
            CGColorRef fillColor = (__bridge CGColorRef)[attributes objectForKey:kQLBackgroundFillColorAttributeName];
            CGFloat cornerRadius = [[attributes objectForKey:kQLBackgroundCornerRadiusAttributeName] floatValue];
            CGFloat lineWidth = [[attributes objectForKey:kQLBackgroundLineWidthAttributeName] floatValue];
           ...
           context:(CGContextRef)c
{
    ...
    for (id line in lines) {        
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            NSDictionary *attributes = (__bridge NSDictionary *)CTRunGetAttributes((__bridge CTRunRef) glyphRun);
            BOOL strikeOut = [[attributes objectForKey:KQLStrikeOutAttributeName] boolValue];
...
- (int)getAttributedStringHeightWidthValue:(int)width
{
    ...
    CTFramesetterRef framesetter = CTFramesetterCreateWithAttributedString((CFAttributedStringRef)self.attributedText);    //string 为要计算高度的NSAttributedString
    ...
    if (self.numberOfLines == 0)
        lastLine = [linesArray count] - 1;
    else
        lastLine = MIN([linesArray count] - 1, self.numberOfLines - 1);

    int line_y = (int) origins[lastLine].y;  //最后一行line的原点y坐标

    CGFloat ascent;
    CGFloat descent;
    CGFloat leading;

    CTLineRef line = (__bridge CTLineRef) [linesArray objectAtIndex:lastLine];
    CTLineGetTypographicBounds(line, &ascent, &descent, &leading);

    CFRelease(textFrame);

    int total_height = 100000 - line_y + (int) descent +1;//+1为了纠正descent转换成int小数点后舍去的值

    return total_height;
}
#pragma mark - QLAttributedLabel
...
- (void)setText:(id)text {
    ...
    if (self.attributedText && self.dataDetectorTypes != UIDataDetectorTypeNone) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            NSArray *results = [self.dataDetector matchesInString:textString options:0 range:NSMakeRange(0, length)];
            if ([results count] > 0) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if ([[self.attributedText string] isEqualToString:textString]) {
...
- (void)setActiveLink:(NSTextCheckingResult *)activeLink {
    ...
    if (_activeLink && [self.activeLinkAttributes count] > 0) {
        if (!self.inactiveAttributedText) {
            //inactiveAttributedText本身是copy的属性，不用再copy
     ...
     limitedToNumberOfLines:(NSInteger)numberOfLines
{
    ...
    if (textSize.height < textRect.size.height) {
        ...
        switch (self.verticalAlignment) {
            case QLAttributedLabelVerticalAlignmentCenter:
                yOffset = floorf((bounds.size.height - textSize.height) / 2.0f);
                break;
            case QLAttributedLabelVerticalAlignmentBottom:
                yOffset = bounds.size.height - textSize.height;
                break;
            case QLAttributedLabelVerticalAlignmentTop:
            default:
                break;
        }
...
- (void)drawTextInRect:(CGRect)rect {
    ...
    if (self.adjustsFontSizeToFitWidth && self.numberOfLines > 0) {
        ...
        if (self.numberOfLines > 1 && self.lineBreakMode == NSLineBreakByWordWrapping) {
            textWidth *= KQLLineBreakWordWrapTextWidthScalingFactor;
        }
    ...
    CGContextSaveGState(c); {
        ...
        if (self.highlightedTextColor && self.highlighted) {
            ...
            [self drawFramesetter:_highlightFramesetter attributedString:highlightAttributedString textRange:textRange inRect:textRect context:c];
        } else {
