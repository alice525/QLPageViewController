 ...
 */
typedef NS_ENUM(NSUInteger, QLAttributedLabelVerticalAlignment) {
    QLAttributedLabelVerticalAlignmentCenter   = 0,
    QLAttributedLabelVerticalAlignmentTop      = 1,
    QLAttributedLabelVerticalAlignmentBottom   = 2,
};

/**
 Determines whether the text to which this attribute applies has a strikeout drawn through itself.
 */
extern NSString * const KQLStrikeOutAttributeName;

/**
 The background fill color. Value must be a `CGColorRef`. Default value is `nil` (no fill).
 */
extern NSString * const kQLBackgroundFillColorAttributeName;

/**
 The background stroke color. Value must be a `CGColorRef`. Default value is `nil` (no stroke).
 */
extern NSString * const kQLBackgroundStrokeColorAttributeName;

/**
 The background stroke line width. Value must be an `NSNumber`. Default value is `1.0f`.
 */
extern NSString * const kQLBackgroundLineWidthAttributeName;

/**
 The background corner radius. Value must be an `NSNumber`. Default value is `5.0f`.
 */
extern NSString * const kQLBackgroundCornerRadiusAttributeName;

@protocol QLAttributedLabelDelegate;

// Override UILabel @property to accept both NSString and NSAttributedString
@protocol QLAttributedLabel <NSObject>
@property (nonatomic, copy) id text;
@end

/**
 `QLAttributedLabel` is a drop-in replacement for `UILabel` that supports `NSAttributedString`, as well as automatically-detected and manually-added links to URLs, addresses, phone numbers, and dates.
 
 # Differences Between `QLAttributedLabel` and `UILabel`
 
 For the most part, `QLAttributedLabel` behaves just like `UILabel`. The following are notable exceptions, in which `QLAttributedLabel` properties may act differently:
 ...
 `QLAttributedLabel`, like `UILabel`, conforms to `NSCoding`. However, if the build target is set to less than iOS 6.0, `linkAttributes` and `activeLinkAttributes` will not be encoded or decoded. This is due to an runtime exception thrown when attempting to copy non-object CoreText values in dictionaries.
...
/**
 ...
 @discussion A `QLAttributedLabel` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */
@property (nonatomic, unsafe_unretained) id <QLAttributedLabelDelegate> delegate;
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `QLAttributedLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) QLAttributedLabelVerticalAlignment verticalAlignment;
...
/**
 The `QLAttributedLabelDelegate` protocol defines the messages sent to an attributed label delegate when links are tapped. All of the methods of this protocol are optional.
 */
@protocol QLAttributedLabelDelegate <NSObject>
 ...
 */
- (void)attributedLabel:(QLAttributedLabel *)label
...
/**
 ...
 @discussion This method is called if no other delegate method was called, which can occur by either now implementing the method in `QLAttributedLabelDelegate` corresponding to a particular link, or the link was added by passing an instance of a custom `NSTextCheckingResult` subclass into `-addLinkWithTextCheckingResult:`.
